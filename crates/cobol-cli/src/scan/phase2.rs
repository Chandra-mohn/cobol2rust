...
// ---------------------------------------------------------------------------
// NDJSON mode: Phase 2 with NDJSON output
// ---------------------------------------------------------------------------

use std::path::Path;
use crate::scan::ndjson::{self, NdjsonWriter};

/// NDJSON Phase 2 result.
struct NdjsonPhase2Result {
    file_id: i64,
    analysis: AnalysisResult,
}

/// Run Phase 2 in NDJSON mode: coverage analysis with NDJSON output.
pub fn run_phase2_ndjson(
    ...
    for chunk in chunks {
        ...
        let results: Vec<NdjsonPhase2Result> = chunk
            .par_iter()
            .filter_map(|(file_id, abs_path)| {
                if shutdown.load(Ordering::Relaxed) {
                    return None;
        ...
        // Write batch to NDJSON on main thread.
        for entry in &results {
            if let Some(ref coverage) = entry.analysis.coverage {
                writer.write_coverage(entry.file_id, run_id, coverage)?;

                if !coverage.unhandled.is_empty() {
                    writer.write_diagnostics(
                        entry.file_id, run_id, 2, &coverage.unhandled, "warning",
                    )?;
                }
            }
            processed_counter.fetch_add(1, Ordering::Relaxed);
